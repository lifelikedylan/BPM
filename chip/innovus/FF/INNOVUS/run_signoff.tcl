####################################################################
# Innovus Foundation Flow Code Generator, Mon Nov  5 08:23:39 CST 2018
...
if {$vars(restore_design)} { restoreDesign DBS/postroute.enc.dat mult_pad }
...
if {[file exists RPT/signoff.summary]} {
   exec /bin/mail -s "FF: mult_pad, signoff completed ([pwd])" < RPT/signoff.summary false
} elseif {[file exists RPT/signoff.summary.gz]} {
   exec gunzip -c RPT/signoff.summary.gz | /bin/mail \
      -s "FF: mult_pad, signoff completed ([pwd])" false
} else {
   exec /bin/mail -s "FF: mult_pad, signoff completed ([pwd])" < /dev/null false
}
