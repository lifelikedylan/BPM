####################################################################
# Innovus Foundation Flow Code Generator, Fri Dec  7 09:19:38 CST 2018
...
if {$vars(restore_design)} { restoreDesign DBS/route.enc.dat bpm_pad }
...
if {[file exists RPT/postroute.summary]} {
   exec /bin/mail -s "FF: bpm_pad, postroute completed ([pwd])" < RPT/postroute.summary false
} elseif {[file exists RPT/postroute.summary.gz]} {
   exec gunzip -c RPT/postroute.summary.gz | /bin/mail \
      -s "FF: bpm_pad, postroute completed ([pwd])" false
} else {
   exec /bin/mail -s "FF: bpm_pad, postroute completed ([pwd])" < /dev/null false
}
