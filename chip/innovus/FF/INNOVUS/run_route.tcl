####################################################################
# Innovus Foundation Flow Code Generator, Wed Nov  7 14:27:13 CST 2018
...
if {$vars(restore_design)} { restoreDesign DBS/postcts_hold.enc.dat mult_pad }
...
if {[file exists RPT/route.summary]} {
   exec /bin/mail -s "FF: mult_pad, route completed ([pwd])" < RPT/route.summary false
} elseif {[file exists RPT/route.summary.gz]} {
   exec gunzip -c RPT/route.summary.gz | /bin/mail \
      -s "FF: mult_pad, route completed ([pwd])" false
} else {
   exec /bin/mail -s "FF: mult_pad, route completed ([pwd])" < /dev/null false
}
