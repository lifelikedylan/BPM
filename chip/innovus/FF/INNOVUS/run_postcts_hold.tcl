####################################################################
# Innovus Foundation Flow Code Generator, Fri Dec  7 09:34:30 CST 2018
...
if {$vars(restore_design)} { restoreDesign DBS/cts.enc.dat bpm_pad }
...
Puts "<FF> MAILING RESULTS TO false"
if {[file exists RPT/postcts_hold.summary]} {
   exec /bin/mail -s "FF: bpm_pad, postcts_hold completed ([pwd])" < RPT/postcts_hold.summary false
} elseif {[file exists RPT/postcts_hold.summary.gz]} {
   exec gunzip -c RPT/postcts_hold.summary.gz | /bin/mail \
      -s "FF: bpm_pad, postcts_hold completed ([pwd])" false
} else {
   exec /bin/mail -s "FF: bpm_pad, postcts_hold completed ([pwd])" < /dev/null false
}
