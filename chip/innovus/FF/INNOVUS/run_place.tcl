####################################################################
# Innovus Foundation Flow Code Generator, Fri Dec  7 09:33:12 CST 2018
...
if {$vars(restore_design)} { restoreDesign DBS/init.enc.dat bpm_pad }
...
if {[file exists RPT/place.summary]} {
   exec /bin/mail -s "FF: bpm_pad, place completed ([pwd])" < RPT/place.summary false
} elseif {[file exists RPT/place.summary.gz]} {
   exec gunzip -c RPT/place.summary.gz | /bin/mail \
      -s "FF: bpm_pad, place completed ([pwd])" false
} else {
   exec /bin/mail -s "FF: bpm_pad, place completed ([pwd])" < /dev/null false
}
