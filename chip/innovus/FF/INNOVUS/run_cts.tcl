####################################################################
# Innovus Foundation Flow Code Generator, Fri Dec  7 09:33:33 CST 2018
...
if {$vars(restore_design)} { restoreDesign DBS/place.enc.dat bpm_pad }
...
if {[file exists RPT/cts.summary]} {
   exec /bin/mail -s "FF: bpm_pad, cts completed ([pwd])" < RPT/cts.summary false
} elseif {[file exists RPT/cts.summary.gz]} {
   exec gunzip -c RPT/cts.summary.gz | /bin/mail \
      -s "FF: bpm_pad, cts completed ([pwd])" false
} else {
   exec /bin/mail -s "FF: bpm_pad, cts completed ([pwd])" < /dev/null false
}
