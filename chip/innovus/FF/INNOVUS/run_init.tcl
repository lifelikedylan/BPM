####################################################################
# Innovus Foundation Flow Code Generator, Fri Dec  7 09:32:54 CST 2018
...
if {[file exists RPT/preplace.summary]} {
   exec /bin/mail -s "FF: bpm_pad, init completed ([pwd])" < RPT/preplace.summary false
} elseif {[file exists RPT/preplace.summary.gz]} {
   exec gunzip -c RPT/preplace.summary.gz | /bin/mail \
      -s "FF: bpm_pad, init completed ([pwd])" false
} else {
   exec /bin/mail -s "FF: bpm_pad, init completed ([pwd])" < /dev/null false
}
